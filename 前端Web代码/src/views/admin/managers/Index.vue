<template>
	<Card class='managers-page'>
		...
		<div class="action-list">
			...
			<Input class="search-text" search type="text" v-model="searchText" placeholder="搜索活动管理员" @change="searchManage"></Input>
		</div>
		...
		<Table class="managers-table" :fit="true" :data="allManagers" :loading="isFetching" ref="activityTable" border stripe>
			...
			<Column class="action-column" label="操作">
				...
				<template slot="scope">
					<Button class="action-btn" type="success" @click="editIt"> 编辑 </Button>
					<Button class="action-btn" type="warning" @click="deleteIt"> 删除 </Button>
				</template>
			</Column>
		</Table>
