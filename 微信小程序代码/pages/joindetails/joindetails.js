...
Page({
  ...
  btns3:function (event){
    ...
    var ul = "http://localhost:8080/link/quit?aid=" + that.data.aid + "&uid= "+ that.data.userInfo.data.id;
    console.log(ul)
    wx.request({
      url: ul,
      data:{
  ...
  onLoad: function () {
    ...
    console.log(that.data.aid)
    var ul = "http://localhost:8080/activities/detail?aid=" 
...
*/
  issubmit(){
    ...
    if(this.data.issu == 0){
      ul = "http://localhost:8808/insert?name=" + userInfo.nickName + "&id=" + this.data.item.id;
      this.setData({
        issu: 1
      })
    }else{
      ul = "http://localhost:8808/delete?name=" + userInfo.nickName + "&id=" + this.data.item.id;
      this.setData({
        issu: 0
      })
    }
